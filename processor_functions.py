...
import shutil 
import time
from particle import PDGID, Particle, Charge # type: ignore
...
def scan_to_df(root_dir, SSXs, llp_id):
    '''Appends SSXs to dataframe. Checks all SSX directories for scan*.txt files and reads them into dataframes,
    then combines into a single dataframe.'''    
                    ...
                    for filename in subfilenames:
                        if filename.startswith('scan') and filename.endswith('.txt'):
                            ...
                            with open(file_path, 'r') as file:
                                ...
                                print(f"Contents of SS file {filename}:")
                                print(df)
    ...
    if all_dataframes:
        ...
        print("No data found.")      ###NB returns None if no SSXs found
...
def awkward_to_pseudojets(awkward_array):
    ...
    for event in awkward_array:
        for particle in range(len(event["px"])):
            ...
            E = event["E"][particle]
            pseudojets.append(fastjet.PseudoJet(px, py, pz, E)) # type: ignore
    return pseudojets
...
def find_llp_pid(llp_filename):
    ...
    with pyhepmc.open(llp_filename) as f: # type: ignore
        for event in f:
